	...
	onFileLoaded(oEvent) {
		...
		const oSheetToJsonOptions = {
			raw: false, 		// To get dates as date string and not int value
			header: aColNames
		};
		...
		const aData = oWorkbook.SheetNames.map(sSheetName => XLSX.utils.sheet_to_json(oWorkbook.Sheets[sSheetName], oSheetToJsonOptions));
	}
